...
// السيرفر → مفتاح آمن + URL مباشر | المتصفح → proxy route (بدون مفتاح)
const BASE_URL = IS_SERVER
  ? (process.env.NEXT_PUBLIC_API_FOOTBALL_BASE || 'https://v3.football.api-sports.io')
  : '/api/football'

const API_KEY = IS_SERVER
  ? (process.env.API_FOOTBALL_KEY || process.env.NEXT_PUBLIC_API_FOOTBALL_KEY)
  : null

// API مفعّل إذا وُجد مفتاح على السيرفر أو تم تفعيل العلم العام
const USE_MOCK = IS_SERVER
  ? (!API_KEY || API_KEY === 'your_api_football_key_here')
...
const api = axios.create({
  ...
  headers: {
    ...
    ...(IS_SERVER && API_KEY ? { 'x-apisports-key': API_KEY } : {}),
  },
})
...
function cached(key, ttlMs, fetcher) {
  ...
  return fetcher().then(data => {
    // لا نخزن null في الكاش — نتيح إعادة المحاولة فوراً
    if (data != null) cache.set(key, { data, ts: now })
    return data
  })
}
...
/**
 * Fetch ALL WC 2026 fixtures (schedule) — cached 5 minutes
 */
export async function fetchAllFixtures() {
  if (USE_MOCK) return []

  return cached('all-fixtures', 5 * 60_000, async () => {
...
export async function fetchMatchDetails(matchId) {
  if (USE_MOCK) return MOCK_MATCHES.find(m => m.id === matchId) || null
  ...
  return cached(`match-${matchId}`, 10_000, async () => {
    ...
    if (!fixtureData) return null   // المباراة غير موجودة أو لم تنطلق بعد

    const match  = normalizeMatch(fixtureData)
    ...
    const lineupsData = lineupsRes.data?.response || []
    match.lineups = {
      home: {
        formation: lineupsData[0]?.formation || '',
        coach:     lineupsData[0]?.coach?.name || '',
        players:   lineupsData[0]?.startXI?.map(mapLP)      || [],
        bench:     lineupsData[0]?.substitutes?.map(mapLP)  || [],
      },
      away: {
        formation: lineupsData[1]?.formation || '',
        coach:     lineupsData[1]?.coach?.name || '',
        players:   lineupsData[1]?.startXI?.map(mapLP)      || [],
        bench:     lineupsData[1]?.substitutes?.map(mapLP)  || [],
      },
    }
    ...
    if (statsData.length >= 2) {
      const h   = statsData[0].statistics || []
      const a   = statsData[1].statistics || []
    ...
    return match
      ...
      console.error('[fetchMatchDetails] error:', err?.message || err)
      return null
    }
  })
}
...
// بناء الترتيب من نتائج المباريات المنتهية (بديل عند غياب بيانات API)
function buildStandingsFromFixtures(fixtures) {
  // بناء خريطة teamId → groupLetter من بيانات GROUPS
  const teamGroupMap = {}
  for (const group of GROUPS) {
    for (const entry of group.teams) {
      teamGroupMap[entry.team.id] = group.id
    }
  }

  // تهيئة الترتيب لكل مجموعة
  const table = {}
  for (const group of GROUPS) {
    table[group.id] = {
      id: group.id,
      name: group.name,
      teams: group.teams.map(e => ({
  ...
  for (const f of (fixtures || [])) {
    ...
    const groupId = teamGroupMap[homeId]
    if (!groupId || !table[groupId]) continue

    const homeEntry = table[groupId].teams.find(t => t.team.id === homeId)
    const awayEntry = table[groupId].teams.find(t => t.team.id === awayId)
    if (!homeEntry || !awayEntry) continue

    const hg = f.homeScore
    const ag = f.awayScore

    homeEntry.played++; homeEntry.gf += hg; homeEntry.ga += ag
    awayEntry.played++; awayEntry.gf += ag; awayEntry.ga += hg

    if (hg > ag) {
      homeEntry.won++; homeEntry.pts += 3; awayEntry.lost++
    } else if (ag > hg) {
      awayEntry.won++; awayEntry.pts += 3; homeEntry.lost++
    } else {
      homeEntry.draw++; homeEntry.pts++
      awayEntry.draw++; awayEntry.pts++
    }
  }
 ...
 * يجرب API أولاً، وعند فشله يحسب الترتيب من نتائج المباريات
 */
export async function fetchStandings() {
  ...
  return cached('standings', 5 * 60_000, async () => {
    try {
      ...
      const apiStandings = res.data.response?.[0]?.league?.standings || []

      if (apiStandings.length > 0) {
        return apiStandings.map(group => {
    ...
    } catch { /* fall through to fixture-based standings */ }

    // إذا كان API فارغاً أو فشل → نحسب الترتيب من نتائج المباريات
    const fixtures = await fetchAllFixtures().catch(() => [])
    return buildStandingsFromFixtures(fixtures)
  })
}
...
/**
 * Fetch official squad for a national team (WC 2026)
 * apiTeamId = API-Football team ID (may differ from app internal ID)
 */
export async function fetchTeamSquad(apiTeamId) {
  if (USE_MOCK || !apiTeamId) return []
  ...
  return cached(`squad-${apiTeamId}`, 60 * 60_000, async () => {
    const res     = await api.get('/players/squads', { params: { team: apiTeamId } })
    ...
    return players.map(p => ({
      ...
      age:    p.age || null,
      number: p.number || 0,
      pos:    posMap[p.position] || p.position || '',
      photo:  p.photo || null,
    }))
  })
}
