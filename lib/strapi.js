...
export async function deleteGroup(groupId) {
  const res = await strapi.delete(`/friend-groups/${groupId}`)
  return res.data
}

export async function removeMemberFromGroup(groupId, userId) {
  // نجلب المجموعة ثم نحذف العضو من القائمة
  const res = await strapi.get(`/friend-groups/${groupId}`, {
    params: { populate: 'members' },
  })
  const group   = res.data.data
  const members = (group.attributes?.members?.data || [])
    .map(m => m.id)
    .filter(id => id !== userId)
  const update = await strapi.put(`/friend-groups/${groupId}`, {
    data: { members },
  })
  return update.data
}
